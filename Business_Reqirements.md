...
## 1. Executive Summary
Forgeline Industries requires a robust **Production Scheduling & Delay Analysis** dashboard to serve as a command center for factory operations. The primary objective of this project is to identify physcial manufacturing bottlenecks and track machine utilization across the product floor.
...
## 4. Scope of Work
**In-Scope:**
  * Ingestion and trandormation of 5,00 synthetic, intentionally messy order records.
...
* **Metric Calculation:** use advacned DAX measures, incorporationg `VAR` and `MAX`, to gracefully handle duplicate records and categorize orders as 'On Time', 'Delayed/ or 'In Progress'.
...
## 6. Non-Functional Requirments
* **Usability & UI/UX:** The dashboard layout must fillow a clean, high-contrast "Card" design over a light gray background.
* **Semantic Coloring:** The visual hierarchy must strictly enforce the Forgeline Industries brand palette: Forgeline Navy (#1E3A5F) for mimary data, Steel Gray (#4B5563) for secondary categories, and Forge Orange (#E85D04) exclusively as an alert color for delayed bottlenecks.
